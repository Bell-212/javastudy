...
public class Library {
  ...
  private void deleteBook() throws RuntimeException {
    ...
    System.out.println(title + " 제목을 가진 책이 없다.");
  }
  ...
  private void modifyBook() throws RuntimeException, InputMismatchException {
    ...
    for(Book book : books) {
      if(title.equals(book.getTitle())) {
        System.out.println("수정할 책의 가격을 입력 >>> ");
        int price = sc.nextInt();
        if(price < 0) {
          throw new InputMismatchException("정확한 가격을 입력하시오.");
        }
        book.setPrice(price);
  ...
  private void queryBook() throws RuntimeException {
    ...
    if(books.isEmpty()) {
      throw new RuntimeException("조회할 책이 없습니다.");
    }
